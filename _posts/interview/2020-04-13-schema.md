...
Possible Rules can be implemented:
...
Select all queries with a particular union of tags
...
Then the document size will become very large. If I load this user object via REST service in my mobile app it could take a while.  Furthermore in worst case the user document could exceed the MongoDb 16MB document limit.


2 Table I: Post Table with Foreign Keys and Tag Table 
...
**2-Table Implementation: Post Table +  Mapping Table**
...
Drawbacks: Not Normalised
More Space in DB
this way you cannot create "unused”/orphan tags so an "add tag" feature has to be performed on an Item. On the other method, the "add tag" feature can be performed independently 

One could argue that the query for the tag title is much faster in a small table which contains each tag only once and this certainly is true. But taking in regard the savings for not having to join and the fact that you can build a good index on them could easily compensate for this
...
Items for one tag (2 table join if we have tag name!)

```
SELECT *
FROM POST
JOIN PostTag ON POSTID
WHERE TAGNAME = 
```
...
Tag-Cloud (2 table join)




All Tags for one item (2 table join if we have id):
...
PostTag(Mapping Table or Relationship Table)
...
Items for one tag (3 table joing if we have tag name)
...
Lots of Tags being the same thing



Tag Table (29488 rows)

tagId
Tagname


TagSynonym Table (1916 rows)

A synonym tag can only be a synonym for one master tag.  master tag can have multiple synonym tags:
...
Why no foreign key on master tag?  Only if you have an information requirement that can't be achieved due to the current technical implementation there would be reason to change or add foreign-keys and possible indexes. 
Foreign keys help by quickly searching based on id, but here we only search or make joins based on tag names.
...
- [Stack Overflow](https://i.stack.imgur.com/AyIkW.png)
