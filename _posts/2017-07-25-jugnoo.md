...
**The Power of a Design Thinking**
...
This failed usability testing. Users - especially new users, were trigger happy -  when they saw a call to action, they would click on it, regardless of whether they had correct checkout information prefilled or not. So we were back to the old design with its  compulsory scrolling, and I was not happy.
...
We changed the Pay button to a Pay 'slider': instead of merely pressing, one had to drag a slider across in order to finish the transaction. This would float at the bottom and was ever presentInterestingly, the users who would unthinkingly press the red button would not do so with a slider!
...
I also shipped two major features on the Driver app: (1) a revamp the onboarding and enrollment process for new drivers to make it much easier for drivers to self-enroll, and (2) an in-app tutorial for drivers which simulated an actual ride: right from first request to the collection of fare. 

However, these features were not used as much as I would have liked, and I never got around to following up on them.  Our driver registrations increased from 10,000 to 22000 by the time I left, but I suspect the credit lay to a sustained enrollment push than due to any product features. 

 

**Conflicts**

Over Samar with AskLocal content. 
with Samar over Jugnoo dogfeeding. 
